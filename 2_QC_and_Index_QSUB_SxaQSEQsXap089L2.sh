...
# To submit this script: qsub -cwd -o logs/2_QC_and_Index_QSUB_SxaQSEQsXap089L2_$(date +%Y%m%d).log -e logs/2_QC_and_Index_QSUB_SxaQSEQsXap089L2_$(date +%Y%m%d).error -S /bin/bash -V -N Submit_2 -q geschwind.q -l h_data=4G,h_rt=1:00:00 2_QC_and_Index_QSUB_SxaQSEQsXap089L2.sh

inParentDir=../data/bam
inLaneDir=${inParentDir}/SxaQSEQsXap089L2
outParentDir=../data/bam/SxaQSEQsXap089L2
...
for sampleDir in ${inLaneDir}/*; do
	...
	if [ ! -f ${outFpath} ]; then
		...
		outLog=logs/2_QC_and_Index_$(date +%Y%m%d)_SxaQSEQsXap089L2_${bnSampleDir}.log
		outErr=logs/2_QC_and_Index_$(date +%Y%m%d)_SxaQSEQsXap089L2_${bnSampleDir}.error

		# Note name of job (-N) will only display 1st 10 characters
		qsub -cwd -o ${outLog} -e ${outErr} -V -S /bin/bash -q geschwind.q -N QC_${bnSampleDir} -l h_data=36G,h_rt=24:00:00 2_QC_and_Index_SxaQSEQsXap089L2.sh ${inParentDir} ${outDir} ${bnSampleDir}
