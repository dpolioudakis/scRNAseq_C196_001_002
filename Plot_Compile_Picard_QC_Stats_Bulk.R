...
# Note: Jason did not run Picard for duplication stats, commented out that section
################################################################################
...
picAlign <- read.table("../metadata/bulk/alignment_summary.txt"
                       , row.names = 1, skip = 1)
# remove unused column names from alignment_stats.txt
cNames <- unlist(strsplit(readLines(con = "../metadata/bulk/alignment_summary.txt"
                             , n = 1), split = " "))
colnames(picAlign) <- cNames[ -c(1, (length(cNames)-2):length(cNames))]

# rnaseq_stats.txt
picSeq <- read.table("../metadata/bulk/rnaseq_stats.txt", skip = 1, row.names = 1)
# remove unused column names from alignment_stats.txt
# (Picard outputs column names for statistics even if they are not calculated)
cNames <- unlist(strsplit(readLines(con = "../metadata/bulk/rnaseq_stats.txt"
                                    , n = 1), split = " "))
...
txCovDF <- read.table("../metadata/bulk/rnaseq_stats_Transcript_Coverage.txt"
                      , fill = TRUE)

# gcbias_summary.txt
gcBiasSummaryDF <- read.table("../metadata/bulk/gcbias_summary.txt", header = TRUE)

# gcbias_stats.txt
gcBiasStatsDF <- read.table("../metadata/bulk/gcbias_stats.txt", header = FALSE)

# # duplication_stats.txt
# # Add NA to blanks
# dupDF <- read.table("../metadata/bulk/duplication_stats.txt"
...
ggplot(ggDF, aes(x = variable, y = value, col = variable)) +
  ...
  xlab("Location") +
  ggtitle(paste0("Plot_Compile_Picard_QC_Stats_Bulk.R"
                 , "\nRNA-seq QC metrics across samples"
                 , "\nPercent of Aligned Bases by Location"
                 , "\n"))
ggsave("../analysis/graphs/Plot_Compile_Picard_QC_Bulk_Percent_Location.pdf"
       , width = 7, height = 7)
################################################################################
...
# Divide by read size
ggDF <- picSeq[, c("PF_BASES", "PF_ALIGNED_BASES", "CODING_BASES", "UTR_BASES", "INTRONIC_BASES", "INTERGENIC_BASES")]/50
...
ggplot(ggDF, aes(x = variable, y = value, col = variable)) +
  ...
  ggtitle(paste0("Plot_Compile_Picard_QC_Stats_Bulk.R"
                 ...
                 , "\n"))
ggsave("../analysis/graphs/Plot_Compile_Picard_QC_Bulk_Aligned_Location.pdf"
       , width = 7, height = 7)
################################################################################
...
pdf("../analysis/graphs/Plot_Compile_Picard_QC_Bulk_Transcript_Coverage.pdf"
    , width = 7, height = 7)
plot(x = as.numeric(colnames(txCovqtlDF)), y = txCovqtlDF[2,]
     ...
     , main = "Plot_Compile_Picard_QC_Stats_Bulk.R
...
# Plots
pdf("../analysis/graphs/Plot_Compile_Picard_QC_Bulk_GC_Bias_Stats.pdf"
    , width = 7, height = 7)
# Normalized coverage by GC %
par(mfrow = c(3,1))
...
# ggsave("../analysis/graphs/Plot_Compile_Picard_QC_Bulk_Percent_Duplication.pdf"
       # , width = 7, height = 7)
################################################################################
...
write.csv(PQCdat,"../metadata/bulk/PicardToolsQC.csv")
