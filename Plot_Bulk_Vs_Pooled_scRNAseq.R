...
plot(picStatsScDF$PF_READS_ALIGNED, apply(scExDatDF, 2, sum))
df <- data.frame(apply(scExDatDF, 2, sum))
df <- merge(df, picStatsScDF[ ,c("X", "PF_READS_ALIGNED")]
            , by.x = "row.names", by.y = "X")
plot(df[ ,2], df[ ,3])
