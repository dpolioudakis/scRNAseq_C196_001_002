...
# Damon Polioudakis
# 2016-01-28
# Script to merge STAR output from multiple lanes (obtained in Stage 1 of the
# pipeline), collect QC information, mark duplicates, and remove reads that # mapped to different chromosomes to produce final .bam file to be used in the # last stage of the pipeline.
...
inDirs=${inParentDir}/SxaQSEQsXap089L2/${bnSampleDir}
echo "In dirs:"
echo ${inDirs}
echo "Out dir:"
echo ${outDir}
...
if [ ! -f ${outDir}/rnaseq_stats.txt ] || [ ! -f ${outDir}/gcbias_summary.txt ] || [ ! -f ${outDir}/PEmatched_markdup_sorted.bai  ]; then
    ...
    if [ ! -f ${outDir}/PEmatched_markdup_sorted.bai ]; then
			...
			samtools view -h -f 0x0002 -b ${outDir}/reordered_duplication_marked_reads.bam > ${outDir}/PEmatched_markdup.bam     ##keep only reads where paired ends map properly together
			echo "all reads left are now only proper pairs"

			${JAV}/java -Xmx4g -Djava.io.tmpdir=${outDir}/tmp2 -jar ${PIC} SortSam INPUT=${outDir}/PEmatched_markdup.bam OUTPUT=${outDir}/PEmatched_markdup_sorted.bam SORT_ORDER=coordinate TMP_DIR=${outDir}/tmp2
			## Sort the marked-duplicates file
			echo "marked-duplicates file is now sorted"

			${JAV}/java -Xmx2g -jar ${PIC} BuildBamIndex INPUT=${outDir}/PEmatched_markdup_sorted.bam
...
if [ -f ${outDir}/PEmatched_markdup_sorted.bai ] && [ -f ${outDir}/gcbias_summary.txt ]; then
        echo "cleaning up the extra .bam files"
	rm ${outDir}/reordered_reads.bam ## Save space... if QC outputs are present, delete the .bam files
	rm ${outDir}/sorted_reads.bai
  rm ${outDir}/sorted_reads.bam
	rm ${outDir}/PEmatched_markdup.bam
